...
from SRC.Functions import dump_data_total, dump_data_8_projects, \
    LoadSavedData, JoinSubLists, SavedData, Generate_saved_path
...
log_path = os.path.join(saved_path, 'logs')
...
def train_model():
    ...
    glove_model = open(w2v_model_path, encoding="utf-8")
    ...
    def train(train_set_x, train_set_y, validation_set_x, validation_set_y, saved_model_name):
        ...
        callbacks_list = [
            ModelCheckpoint(
                filepath=os.path.join(model_saved_path, Version + saved_model_name +
            ...
            CSVLogger(
                os.path.join(log_path,
                             saved_model_name + datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '.log'))]

        train_history = model.fit(train_set_x, train_set_y,
                                  epochs=EPOCHS,
                                  batch_size=BATCH_SIZE,
                                  shuffle=False,
                                  # The data has already been shuffle before, so it is unnessary to shuffle it again.
                                  # (And also, we need to correspond the ids to the features of the samples.)
    ...
    def plot_history(network_history):
        ...
        plt.savefig(os.path.join(model_saved_graph_path, Version + model_name + '_Epoch_loss' + '.png'))
        # plt.show()
    ...
    for i in range(LoopN):
        ...
        SavedData(os.path.join(model_saved_pkl_path, Version + model_name + '.pkl'), train_history)
